...
if (empty($_POST['name']) || !isset($_POST['name'])) {
  header("location: ./admin_glowna.php?cl=DodajMenegera&error=Podaj imie");
  exit();
}elseif (empty($_POST['surname']) || !isset($_POST['surname'])) {
  header("location: ./admin_glowna.php?cl=DodajMenegera&error=Podaj nazwisko");
  exit();
}elseif (empty($_POST['email']) || !isset($_POST['email'])) {
  header("location: ./admin_glowna.php?cl=DodajMenegera&error=Podaj email");
  exit();
}elseif (empty($_POST['dataur']) || !isset($_POST['dataur'])) {
  header("location: ./admin_glowna.php?cl=DodajMenegera&error=Podaj date urodzenia");
  exit();
}elseif (empty($_POST['miejscowosc']) || !isset($_POST['miejscowosc'])) {
  header("location: ./admin_glowna.php?cl=DodajMenegera&error=Podaj miejscowosc");
  exit();
}
...
if ($connect->query($sql)) {
  ...
  try {
    ...
    $mail->send();
    $connect->close();
    header("location: ./admin_glowna.php?cl=DodajMenegera&error=Poprawnie dodano usera");
    exit();
  } catch (Exception $e) {
    header("location: ./admin_glowna.php?cl=DodajMenegera&error=Błąd wysyłania maila, dodano usera");
  }
}
else {
  header("location: ./admin_glowna.php?cl=DodajMenegera&error=Nie dodano usera $code, $account_number");
  $connect->close();
}
